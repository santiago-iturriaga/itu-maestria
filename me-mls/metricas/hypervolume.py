...
def full_instancias(datos):
    (INSTANCES,SCENARIOS,WORKLOADS,TRUE_PF,BASE_PATH,FILE_PREFIX,OUTPUT_PATH) = datos
        ...
        for s in SCENARIOS:
            ...
            for i in range(INSTANCES):
                ...
                if len(true_pf) > 0 and len(instance_pf):
                    ...
                    #print OUTPUT_PATH+"/"+FILE_PREFIX+"scenario."+str(s)+".workload."+w+"."+str(i)+".hv"
                                      
                    #pals-aga.scenario.19.workload.A.u_i_lolo.2.hv
                    with open(OUTPUT_PATH+"/"+FILE_PREFIX+"scenario."+str(s)+".workload."+w+"."+str(i)+".hv", "w") as o_file:
...
def full_dimensiones():
    ...
    TRUE_PF = "../"+DIMENSION+".fp"
    BASE_PATH = "../"+DIMENSION+".24_10s"
    FILE_PREFIX = "pals-aga."
    OUTPUT_PATH = DIMENSION+"/metrics-aga"

    aga = full_instancias((INSTANCES,SCENARIOS,WORKLOADS,TRUE_PF,BASE_PATH,FILE_PREFIX,OUTPUT_PATH))

    print DIMENSION+" ADHOC"

    TRUE_PF = "../"+DIMENSION+".fp"
    BASE_PATH = "../"+DIMENSION+".24.adhoc"
    FILE_PREFIX = "pals-1."
    OUTPUT_PATH = DIMENSION+"/metrics-adhoc"

    adhoc = full_instancias((INSTANCES,SCENARIOS,WORKLOADS,TRUE_PF,BASE_PATH,FILE_PREFIX,OUTPUT_PATH))
