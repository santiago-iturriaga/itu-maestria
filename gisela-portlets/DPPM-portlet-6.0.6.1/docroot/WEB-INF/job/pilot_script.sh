...
#INFILE=$(ls -1 | grep entrada)
INFILE=$(find . -maxdepth 1 -type f | grep entrada | xargs basename)
X=$(echo $INFILE | awk -F"_" '{ print $3 }')
#ln -s $INFILE entradas/$X
cp $INFILE $X
cp $X entradas/
INPARAM=$(echo $X | awk -F"." '{ print $1 }' | sed s/entrada//)
echo "INFILE: $INFILE"
echo "X: $X"
...
# Execute
chmod +x DeadLineProblem_seq
./DeadLineProblem_seq $INPARAM $THETA >> $OUTFILE
...
#
tar cvfz DPPM-Files.tar.gz $INFILE $OUTFILE salidas entradas
