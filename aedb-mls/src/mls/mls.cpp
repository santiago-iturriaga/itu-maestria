...
void mls_init(int seed, struct mls_instance &empty_instance)
{
    ...
    empty_instance.work_type = MLS__INIT;
...
void* mls_thread(void *thread_arg)
{
    ...
    int work_iterations;
    work_iterations = (int)floor(MLS__THREAD_ITERATIONS / MLS__THREAD_RE_WORK_FACTOR);
    ...
    while ((terminate == 0) && (thread_instance->global_total_iterations[0] < thread_instance->max_iterations))
    {
        ...
        else if (work_type == MLS__INIT)
        {
            ...
            pthread_mutex_lock(thread_instance->population_mutex);
                thread_instance->work_type[0] = MLS__SEARCH;
        ...
        else if (work_type == MLS__SEARCH)
        {
            ...
            else
            {
                ...
                random = cpu_mt_generate(*(thread_instance->thread_random_state));
                work_iteration_size = (int)(work_iterations + (random * (MLS__THREAD_ITERATIONS - work_iterations)));
                ...
                while (work_do_iteration == 1) {
                    ...
                    if (1 == 1) // TODO!!!
                    {
                        ...
                        if (mutex_locked == 0) {
                            ...
                            random = cpu_mt_generate(*(thread_instance->thread_random_state));
                            work_iteration_size = (int)floor(random * work_iterations);
