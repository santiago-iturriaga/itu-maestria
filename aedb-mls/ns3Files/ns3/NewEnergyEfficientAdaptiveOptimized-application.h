...
#ifndef __NewEnergyEfficientAdaptiveOptimized_application_h__
#define __NewEnergyEfficientAdaptiveOptimized_application_h__
 ...
 * \defgroup NewEnergyEfficientAdaptiveOptimizedApplication
 *
 * Creates a NewEnergyEfficientAdaptiveOptimized tree topology
 * Author: Patricia Ruiz
**/

class NewEnergyEfficientAdaptiveOptimizedApplication : public Application 
...
public:
  ...
  NewEnergyEfficientAdaptiveOptimizedApplication ();
  virtual ~NewEnergyEfficientAdaptiveOptimizedApplication();
...
private:
	...
	double CalculateNeighborsNotSenderPower();
	int CalculateBorderNeighbors();
	double CalculateBorderNeighborsEnergy();
	//void Contains(Mac48Address value, std::vector<Mac48Address> vector);
	...
	TypeId          m_tid;
	double          m_pwRxDbm;
    Ptr<Socket>     m_NewEnergyEfficientAdaptiveOptimizedSocket; // Associated socket
	Ipv4Address     m_addr; 	
	...
	double          m_neighborsBorderThreshold;
	double          m_energyThreshold;
	double          m_marginThreshold;
	int             m_neighborThreshold;
	...
	std::vector<Mac48Address> m_neighborDiscardedList;
