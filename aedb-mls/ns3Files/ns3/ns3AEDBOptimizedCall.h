...
class ns3AEDBOptimizedCall
{
	typedef std::vector<GlobalValue *> VectorA;
	typedef VectorA::const_iterator Iterator;
public:
	ns3AEDBOptimizedCall ();
	//ExperimentJNA (std::string name);
	double* RunExperimentAEDBOptimized (int numberDevices, int j,double minDelay, double maxDelay, double borderThreshold, double marginThreshold,int neighborsThreshold);//,char* name);//double &energy_used, int &total_coverage, double &broadcast_time);//, double *energy, double *forward );
	...
	void CreateSimulation (ns3AEDBOptimizedCall exp, int numberDevices, double minDelay, double maxDelay, double borderThreshold, double marginThreshold,int neighborsThreshold);
