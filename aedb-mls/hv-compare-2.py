...
def main():
    if len(sys.argv) != 7:
        print("[ERROR] Usage: {0} <best PF> <moea1 PF> <num. exec.> <moea2 PF> <num. exec.> <min. coverage>".format(sys.argv[0]))
        exit(-1)

    best_pf_file = sys.argv[1]
    moea1_pf_file = sys.argv[2]
    moea1_num_exec = int(sys.argv[3])
    moea2_pf_file = sys.argv[4]
    moea2_num_exec = int(sys.argv[5])
    min_cover = float(sys.argv[6])

    print("Best PF file    : {0}".format(best_pf_file))
    print("MOEA1 PF file   : {0} ({1})".format(moea1_pf_file, moea1_num_exec))
    print("MOEA2 PF file   : {0} ({1})".format(moea2_pf_file, moea2_num_exec))
    ...
    moea1_pf = []
    moea1_pf_value = []
    for e in range(moea1_num_exec):
        moea1_pf_exec = []

        with open(moea1_pf_file + "." + str(e)) as f:
            for line in f:
                if len(line.strip()) > 0:
                    data = line.strip().split("\t")
                    ...
                    if len(data) == 3:
                        ...
                        if coverage > min_cover and energy > 0:
                            moea1_pf_exec.append((energy,1/coverage,nforwardings))
                                
        moea1_pf.append(moea1_pf_exec)
        
        normalized_pf = getNormalizedFront(moea1_pf_exec, maxValues, minValues)
        inverted_pf = invertedFront(normalized_pf)
        hv = hypervolume(inverted_pf,len(inverted_pf), len(inverted_pf[0]))
        
        moea1_pf_value.append(hv)

    moea2_pf = []
    moea2_pf_value = []
    for e in range(moea2_num_exec):
        moea2_pf_exec = []

        with open(moea2_pf_file + "." + str(e)) as f:
            for line in f:
                if len(line.strip()) > 0:
                    data = line.strip().split("\t")
                    ...
                    if len(data) == 3:
                        ...
                        if coverage > min_cover and energy > 0:
                            moea2_pf_exec.append((energy,1/coverage,nforwardings))
                                
        moea2_pf.append(moea2_pf_exec)
        
        normalized_pf = getNormalizedFront(moea2_pf_exec, maxValues, minValues)
        inverted_pf = invertedFront(normalized_pf)
        hv = hypervolume(inverted_pf,len(inverted_pf), len(inverted_pf[0]))
        
        moea2_pf_value.append(hv)
    ...
    (moea1_avg, moea1_stdev) = avg_stdev(moea1_pf_value)
    (moea2_avg, moea2_stdev) = avg_stdev(moea2_pf_value)
    (hstatic, pvalue) = mstats.kruskalwallis(moea1_pf_value, moea2_pf_value)
    (u, prob) = stats.mannwhitneyu(moea1_pf_value, moea2_pf_value)
    
    print("alg|avg|stdev")
    print("moea1|{0}|{1}".format(moea1_avg, moea1_stdev))
    print("moea2|{0}|{1}".format(moea2_avg, moea2_stdev))
