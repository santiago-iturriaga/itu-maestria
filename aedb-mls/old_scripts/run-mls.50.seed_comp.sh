...
time(~/bin/mpich2-1.4.1p1/bin/mpiexec -launcher ssh -launcher-exec oarsh -f $OAR_NODEFILE -np 9 -ppn 1 bin/mls.seed_comp $RANDOM $ITERATIONS $THREADS $SIMULS $INSTANCE)
