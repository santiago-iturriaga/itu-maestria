    ...
    while (nro_asig < NT)
    {
        ...
        for (i=0;i<NT;i++)
        {
            ...
            if (asig[i] == NO_ASIG)
            {
                ...
                for (j=0;j<NM;j++)
                {
                    ...
                    if (et <= mak_temp) {
                        ...
                        for (int aux_j = 0; aux_j < NM; aux_j++) {
                            if (aux_j == j) {
                                delta_energy += (ETC[i][j] * E_MAX[j]) - ((mak_temp - mach[j]) * E_IDLE[j]);
                            } else {
                ...
                mct_i_j = mach[best_mach_task]+ETC[i][best_mach_task];
        ...
        if (mach[best_machine] > mak_temp)
        {
            ...
            for (j=0;j<NM;j++)
            {
                if ( j != best_machine )
                {
                    energy_mach[j] = (mach[j] * E_MAX[j]) + (E_IDLE[j] * (mak_temp - mach[j]));
                } else {
                    energy_mach[j] = mach[j] * E_MAX[j];
                }
            }
        } else {
            energy_mach[best_machine] += (ETC[best_task][best_machine] * E_MAX[j]);
        }
    }
