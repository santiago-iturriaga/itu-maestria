...
#include "pals_gpu_rmachine.h"

// ==============================================================================================
// NOTA. Debido al uso del generador de números aleatorios RNG_rand48:
// PALS_GPU_RMACHINE__BLOCKS * PALS_GPU_RMACHINE__LOOPS_PER_THREAD = debe ser mútiplo de 6144 (1024*6).
// ==============================================================================================
//#define PALS_GPU_RMACHINE__BLOCKS 			2048
//#define PALS_GPU_RMACHINE__THREADS 			128
//#define PALS_GPU_RMACHINE__LOOPS_PER_THREAD 	128

#define PALS_GPU_RMACHINE__BLOCKS 				1024
#define PALS_GPU_RMACHINE__THREADS 				128
#define PALS_GPU_RMACHINE__LOOPS_PER_THREAD 	64

#define PALS_GPU_RMACHINE__MOV_TYPE_OFFSET		PALS_GPU_RMACHINE__THREADS * PALS_GPU_RMACHINE__LOOPS_PER_THREAD

__global__ void pals_rmachine_kernel(int machines_count, int tasks_count, int tasks_per_thread, float *gpu_etc_matrix, 
...
void pals_gpu_rmachine_wrapper(struct matrix *etc_matrix, struct solution *s, 
	...
	for (int i = 0; i < instance.result_count; i++) {
		...
		int mov_type_offset = PALS_GPU_RMACHINE__MOV_TYPE_OFFSET;
		int swap = best_swaps[block_idx];

		int move_type = swap / mov_type_offset;
		int move_offset = swap % mov_type_offset;
	
		int thread_idx = move_offset % PALS_GPU_RMACHINE__THREADS;
		int loop_idx = move_offset / PALS_GPU_RMACHINE__THREADS;

		int r_block_offset_start = block_idx * (2 * PALS_GPU_RMACHINE__LOOPS_PER_THREAD);

		if (move_type == PALS_GPU_RMACHINE_SWAP) { // Movement type: SWAP
			int random_1 = rands_nums[r_block_offset_start + loop_idx] % etc_matrix->tasks_count;
			int task_x = random_1;
	
			int random_2 = rands_nums[r_block_offset_start + loop_idx + PALS_GPU_RMACHINE__BLOCKS];
			int task_y = random_2 % (etc_matrix->tasks_count - 1 - PALS_GPU_RMACHINE__THREADS) + thread_idx;
			...
			// <======= DEBUG
		} else if (move_type == PALS_GPU_RMACHINE_MOVE) { // Movement type: MOVE
			int random_1 = rands_nums[r_block_offset_start + loop_idx] % etc_matrix->tasks_count;
			int task_x = random_1;
			int machine_a = s->task_assignment[task_x];
