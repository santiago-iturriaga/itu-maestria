...
#include "pals_gpu_randTask.h"

#define THREADS_PER_BLOCK 128
#define MIN_TASKS_PER_THREAD 4
#define MAX_BLOCKS 1024

__global__ void pals_randTask_kernel(int task_count, int block_size, int tasks_per_thread, float *gpu_etc_matrix, 
	int *gpu_task_assignment, int *gpu_best_swaps, float *gpu_best_swaps_delta);

void pals_gpu_randTask_init(struct matrix *etc_matrix, struct solution *s, struct pals_gpu_randTask_instance *instance) {
...
void pals_gpu_randTask_finalize(struct pals_gpu_randTask_instance *instance) {
	cudaFree(instance->gpu_etc_matrix);
	cudaFree(instance->gpu_task_assignment);
	cudaFree(instance->gpu_best_swaps);
}

void pals_gpu_randTask_wrapper(struct matrix *etc_matrix, struct solution *s, struct pals_gpu_randTask_instance *instance, 
	...
	*/
	...
	pals_randTask_kernel<<< grid, threads >>>(
		etc_matrix->tasks_count, 
		instance->block_size, 
		instance->tasks_per_thread, 
...
__global__ void pals_randTask_kernel(int task_count, int block_size,
