...
do
	...
	echo "... minmin"
	time (${BASE_PATH}/bin/pals_cpu ${INSTANCE_PATH}/${INSTANCE} ${DIM_SIZE} 4 ${THREAD_COUNT} 0 > ${SOL_PATH}/${INSTANCE}.minmin.sol) 2> ${SOL_PATH}/${INSTANCE}.minmin.time
	${BASE_PATH}/bin/verificador ${INSTANCE_PATH}/${INSTANCE} ${SOL_PATH}/${INSTANCE}.minmin.sol ${DIM_SIZE} > ${SOL_PATH}/${INSTANCE}.minmin.makespan
	
	echo "... mct"
	time (${BASE_PATH}/bin/pals_cpu ${INSTANCE_PATH}/${INSTANCE} ${DIM_SIZE} 5 ${THREAD_COUNT} 0 > ${SOL_PATH}/${INSTANCE}.mct.sol) 2> ${SOL_PATH}/${INSTANCE}.mct.time
