...
if __name__ == '__main__':  
    #corpus = nltk.corpus.conll2002.sents('esp.train')
    #corpus.extend(nltk.corpus.conll2002.sents('esp.testa'))
    #corpus.extend(nltk.corpus.conll2002.sents('esp.testb'))
    
    total = len(nltk.corpus.conll2002.sents('esp.train'))
    step = total / 10
    #print "TOTAL: %s CORPUS" % len(corpus)
    
    train_set = nltk.corpus.conll2002.sents('esp.train')[:step*9]
    test_set = nltk.corpus.conll2002.sents('esp.train')[-step:]
    print "TOTAL: %s TRAIN" % len(train_set)
    print "TOTAL: %s TEST" % len(test_set)
    
    print ">> Training..."
    examples = build_set(train_set)  
