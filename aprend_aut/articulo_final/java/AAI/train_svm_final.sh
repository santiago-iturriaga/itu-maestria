export path=$path:/home/santiago/Facultad/AAI/SVMTool-1.3.1/bin
export PERL5LIB=/home/santiago/Facultad/AAI/SVMTool-1.3.1/lib:$PERL5LIB

cd model_svm

i=2
m=0

echo "NAME = final_svm" > final_svm.svmt
echo "TRAINSET = /home/santiago/eclipse/java-workspace/AAI/corpus/train_2.txt" >> final_svm.svmt
echo "TESTSET = /home/santiago/eclipse/java-workspace/AAI/corpus/test_full_2.txt" >> final_svm.svmt
echo "SVMDIR = /home/santiago/Facultad/AAI/svm_light/" >> final_svm.svmt
echo "REMOVE_FILES = 1" >> final_svm.svmt
echo "X = 10" >> final_svm.svmt
echo "W = 5 2" >> final_svm.svmt
echo "Kfilter = 0" >> final_svm.svmt
echo "Ufilter = 0" >> final_svm.svmt

#DEFAULT
echo "A0k = w(-2) w(-1) w(0) w(1) w(2) w(-2,-1) w(-1,0) w(0,1) w(-1,1) w(1,2) w(-2,-1,0) w(-2,-1,1) w(-1,0,1) w(-1,1,2) w(0,1,2) p(-2) p(-1) p(-2,-1) p(-1,1) p(1,2) p(-2,-1,1) p(-1,1,2) a(0) a(1) a(2) m(0) m(1) m(2)" >> t_full_a_${i}_${m}.svmt
echo "A0u = w(-2) w(-1) w(0) w(1) w(2) w(-2,-1) w(-1,0) w(0,1) w(-1,1) w(1,2) w(-2,-1,0) w(-2,-1,1) w(-1,0,1) w(-1,1,2) w(0,1,2) p(-2) p(-1) p(-2,-1) p(-1,1) p(1,2) p(-2,-1,1) p(-1,1,2) a(0) a(1) a(2) m(0) m(1) m(2) a(2) a(3) a(4) z(2) z(3) z(4) ca(1) cz(1) L SA AA SN CA CAA CP CC CN MW" >> t_full_a_${i}_${m}.svmt

#MOD1
#echo "A0k = w(-2) w(-1) w(0) w(1) w(2) w(-2,-1) w(-1,0) w(0,1) w(-1,1) w(1,2) w(-2,-1,0) w(-2,-1,1) w(-1,0,1) w(-1,1,2) w(0,1,2) p(-2) p(-1) p(-2,-1) p(-1,1) p(1,2) p(-2,-1,1) p(-1,1,2) a(0) a(1) a(2) m(0) m(1) m(2)" >> t_full_a_${i}_${m}.svmt
#echo "A0u = w(-2) w(-1) w(0) w(1) w(2) w(-2,-1) w(-1,0) w(0,1) w(-1,1) w(1,2) w(-2,-1,0) w(-2,-1,1) w(-1,0,1) w(-1,1,2) w(0,1,2) p(-2) p(-1) p(-2,-1) p(-1,1) p(1,2) p(-2,-1,1) p(-1,1,2) a(0) a(1) a(2) m(0) m(1) m(2) a(2) a(3) a(4) z(2) z(3) z(4) ca(1) cz(1)" >> t_full_a_${i}_${m}.svmt

#MOD2
#echo "A0k = w(-2) w(-1) w(0) w(1) w(2) w(-2,-1) w(-1,0) w(0,1) w(-1,1) w(1,2) w(-2,-1,0) w(-2,-1,1) w(-1,0,1) w(-1,1,2) w(0,1,2) p(-2) p(-1) p(-2,-1) p(-1,1) p(1,2) p(-2,-1,1) p(-1,1,2) a(0) a(1) a(2) m(0) m(1) m(2) a(2) a(3) a(4) z(2) z(3) z(4) ca(1) cz(1) sentence_info L MW CC CN CP SN AA aa CA SA sa" >> t_full_a_${i}_${m}.svmt
#echo "A0u = w(-2) w(-1) w(0) w(1) w(2) w(-2,-1) w(-1,0) w(0,1) w(-1,1) w(1,2) w(-2,-1,0) w(-2,-1,1) w(-1,0,1) w(-1,1,2) w(0,1,2) p(-2) p(-1) p(-2,-1) p(-1,1) p(1,2) p(-2,-1,1) p(-1,1,2) a(0) a(1) a(2) m(0) m(1) m(2) a(2) a(3) a(4) z(2) z(3) z(4) ca(1) cz(1) sentence_info L MW CC CN CP SN AA aa CA SA sa" >> t_full_a_${i}_${m}.svmt

echo "do M${m} LRL" >> final_svm.svmt

/home/santiago/Facultad/AAI/SVMTool-1.3.1/bin/SVMTlearn final_svm.svmt

cd ..
