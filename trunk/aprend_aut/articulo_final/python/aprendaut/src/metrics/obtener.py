...
def obtenerMetricasCRF():
    ...
    for adv in sorted(metricas_por_adv.keys()):
        ...
        if metricas_por_adv[adv]['total'] > 0:
            ...
            resultado_metricas_por_adv[adv] = (precision_adv, recall_adv, f_measure_adv, metricas_por_adv[adv]['total'], adv)
        
    return (precision_O, recall_O, f_measure_O, precision_SIN_TILDE, recall_SIN_TILDE, f_measure_SIN_TILDE, \
            precision_CON_TILDE, recall_CON_TILDE, f_measure_CON_TILDE, resultado_metricas_por_adv)

def obtenerMetricasSVM():
    O_tp = 0
    O_fp = 0
    O_fn = 0
...
if __name__ == '__main__':
    svm = obtenerMetricasSVM()
    crf = obtenerMetricasCRF()

    print "=========================================================================================================="

    print "\\texttt{O} & %.2f & %.2f & %.2f & %.2f & %.2f & %.2f \\\\" % (svm[0], crf[0], svm[1], crf[1], svm[2], crf[2])
    print "\\texttt{SIN\\_TILDE} & %.2f & %.2f & %.2f & %.2f & %.2f & %.2f \\\\" % (svm[3], crf[3], svm[4], crf[4], svm[5], crf[5])
    print "\\texttt{CON\\_TILDE} & %.2f & %.2f & %.2f & %.2f & %.2f & %.2f \\\\" % (svm[6], crf[6], svm[7], crf[7], svm[8], crf[8])

    print "=========================================================================================================="

    assert(len(svm[-1]) == len(crf[-1]))

    for adv_aux in sorted(svm[-1].values(), key=itemgetter(-2), reverse=True):
        adv = adv_aux[-1]
        
        if palabra_sin_tilde(adv) == adv:
            print "%s & %d & %.2f & %.2f & %.2f & %.2f & %.2f & %.2f \\\\" % (adv, svm[-1][adv][3], svm[-1][adv][0], crf[-1][adv][0], svm[-1][adv][1], crf[-1][adv][1], svm[-1][adv][2], crf[-1][adv][2])
