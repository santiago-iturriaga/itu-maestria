...
public class OffsetFeatureConjunction extends Pipe implements Serializable {
	 ...
	 * 
	 * @param thisFeatureName
	 *            Name of this conjunction feature.
	 * @param featureNames
	 *            String giving name for each subfeature i.
	 * @param offsets
	 *            For each subfeature i, which offset from the current timestep
	 *            must i appear at.
	 * @param isNonNegated
	 *            If element i is false, then the negation of the feature is
	 *            added to the conjuction.
	 */
	public OffsetFeatureConjunction(String thisFeatureName,
			String[] featureNames, int[] offsets, boolean[] isNonNegated,
	...
	public OffsetFeatureConjunction(String thisFeatureName,
			String[] featureNames, int[] offsets, boolean tagAllTimesteps) {
		this(thisFeatureName, featureNames, offsets,
				trueArray(featureNames.length), tagAllTimesteps);
	}

	public OffsetFeatureConjunction(String thisFeatureName,
			String[] featureNames, int[] offsets) {
		this(thisFeatureName, featureNames, offsets,
				trueArray(featureNames.length), false);
	}
	...
	public Instance pipe(Instance carrier) {
		...
		for (int t = 0; t < tsSize; t++) {
			...
			for (int fnum = 0; fnum < featurePatterns.length; fnum++) {
				...
				boolean featurePresent = hasMatchingFeature(ts.get(pos),
						featurePatterns[fnum]);
	...
	private void writeObject(ObjectOutputStream out) throws IOException {
		...
		int size;
		size = (featurePatterns == null) ? NULL_INTEGER
	...
	private void readObject(ObjectInputStream in) throws IOException,
		...
		if (version >= 1)
			tagAllTimesteps = in.readBoolean();
