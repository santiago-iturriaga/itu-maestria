...
import cc.mallet.pipe.tsf.SVMPrintingPipe;
...
public class OutputSVMTrainFinal {

	private OutputSVMTrainFinal() {
	}
	...
	public static CRF TrainCRF(String trainingFilename, String log)
			throws IOException {
		...
		pipes.add(new SVMPrintingPipe(log));
		pipes.add(new OutputSVMTrainFinal.SimpleTokenSentence2FeatureVectorSequence());
...
//		String train = "corpus/test_full_1.txt";
...
//		PrintWriter log = new PrintWriter(output);
		...
		if (!modelFile.exists()) {
			CRF modelObj = TrainCRF(train, output);
